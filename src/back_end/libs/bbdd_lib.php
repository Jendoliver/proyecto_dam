    ...
    if($row = mysqli_fetch_assoc($res)) //comprobamos que hay algo para evitar warning
    {
        ...
        do // llenar tabla con el contenido de la query
        {
            ...
            switch($button) // preparamos los botones según el caso
            {
                ...
                case 32: $table .= "<td id='$idconcierto'><button class='modifyconcert btn btn-sm btn-primary'>MODIFICAR</button> <button class='deleteconcert btn btn-sm btn-danger' data-toggle='modal' data-target='#modaldeleteconcert'>ELIMINAR</button></td>"; break; // boton modificar concierto
            }
            $table .= "</tr>";
